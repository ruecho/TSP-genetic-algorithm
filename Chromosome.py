...
with open(file_name, "r") as f:
    for line in f:  # check each line
        new_line = line.strip()  # removes spaces at the beginning and the end if they are available
        new_line = new_line.split(" ")  # splits by spaces. so we get the datas
        id, x, y = new_line[0], new_line[1], new_line[2]  
        dataset.append(Node(id=id, x=x, y=y))  # Create a Node object with id, x, y and add to the dataset list

N = 20  # Total number of points/cities
...
matrix = create_distance_matrix(dataset)  # send our dataset as a parameter to the function. so we get the distance matrix for cities
